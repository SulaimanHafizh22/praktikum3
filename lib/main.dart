...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<int> addSahams() async {
    MySaham tlkm = MySaham(ticker: "TLKM", open: 3380, high: 3500, last: 3490, change: "2,05");
    MySaham ammn = MySaham(ticker: "AMMN", open: 6750, high: 6750, last: 6500, change: "-3,7");
    MySaham bren = MySaham(ticker: "BREN", open: 4500, high: 4610, last: 4580, change: "1,78");
    MySaham cuan = MySaham(ticker: "CUAN", open: 5200, high: 5525, last: 5400, change: "3,85");

    List<MySaham> listOfSahams = [tlkm, ammn, bren, cuan];

    return await this.handler.insertMySaham(listOfSahams);
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      if (_formKey.currentState!.validate()) {
                        ...
                        await handler.insertMySaham(newSaham);
          ...
          Expanded(
            child: FutureBuilder<List<MySaham>>(
              future: handler.retrieveSahams(),
              builder: (BuildContext context, AsyncSnapshot<List<MySaham>> snapshot) {
                if (snapshot.hasData) {
                  return ListView.builder(
                    itemCount: snapshot.data?.length,
                    itemBuilder: (BuildContext context, int index) {
                      MySaham saham = snapshot.data![index];
                      return ListTile(
